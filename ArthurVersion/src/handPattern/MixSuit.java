...
public class MixSuit extends HandPattern {
	...
	public boolean checkPattern(Hand hand) {
		...
		if (tileSet.get(0).getSuit() == Suits.Bamboo || tileSet.get(0).getSuit() == Suits.Character
		...
		for (int i = 0; i < tileSet.size(); i++) {
			if (tileSet.get(i).getRank() != '0' && tileSet.get(i).getSuitName() != checkingSuit) {
				return false; // rank 0 means it is not Character/Bamboo/Dot
			}
		}
		...
		return false; // not mix suit (all number)
	}
}
